...
# The relocation guard lives in a here-string and the readiness parameters live in the entry
# script's param block, so the raw-source assertions below span the whole partition too.
...
$setupAsts = @(foreach ($setupFile in $setupSourceFiles) {
        ...
        Assert-True ($parseErrors.Count -eq 0) "$setupFile must parse before its relocation path can be tested."
        $fileAst
    })
